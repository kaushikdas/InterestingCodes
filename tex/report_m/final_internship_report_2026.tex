...
\fancyhead[R]{\small Maharnab Das, IIT Kharagpur}
\fancyfoot[C]{\thepage}
...
\begin{titlepage}
  ...
  \begin{tabular}{r@{\hskip 0.5em}l}
    ...
    \textbf{Programme:}  & Research Intern                             \\[5pt]
...
budget of 1000 environment steps. All experiments use the fixed
start--goal pair $s_0 = (x_0, y_0) = (1.05, 5.92)$ and
$s_g = (x_g, y_g) = (7, 9)$, with geodesic path length
$L_{\mathrm{phys}} \approx 6.70$\,units. At $T = 384$, the required
mean displacement is $\bar{d} = L_{\mathrm{phys}}/T \approx 0.017$,
which falls below the minimum representable step size
$d_{\min} = 0.035$, confirming that the distance-burning condition
analysed in Section~\ref{sec:fm3} is active for this configuration.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{plots/comparison_plot.png}
  \caption{The \texttt{maze2d-large-v1} environment, with rollouts
    from all three architectures overlaid. PG-CD (green) maintains
    tight, constraint-respecting curvature throughout, while UCD
    (blue) intersects the obstacle boundary.}
...
endpoints provides the natural central value that maximises the
...
\begin{equation}
  ...
  \label{eq:phase_lag}
\end{equation}
which is uniformly negative (phase-lagging) and attains its maximum
magnitude near $\omega = \pi/2$.
...
residual distance through path extension. Across the 15-pair evaluation set, we empirically observe that this distance burning results in execution path lengths up to $10\times$ longer than the simplified path (e.g., requiring 521 steps versus 52 steps on Pair 2 before loop removal).\\[4pt]
...
along the geodesic. It instead generates a \textbf{non-monotone
...
size~1 (single-episode inference). The final benchmark evaluates
each architecture over $n = 3$ episodes, with fixed start position
$(x_0, y_0) = (1.05,\, 5.92)$ and goal $(x_g, y_g) = (7,\, 9)$, so
as to ensure direct comparability across architectures. The
progressive ablation reported in Section~\ref{sec:benchmark}
uses an evaluation set of $n = 15$ diverse start--goal pairs, stratified to
include both short hallways and challenging cross-maze trajectories (such as Pair 2).
The metrics reported are defined formally in Table~\ref{tab:metrics}.
...
\caption{Progressive ablation: success rate and execution
  steps under incremental application of fixes.
  ``---'' denotes episodes that did not terminate
...
The present findings constitute a critical \emph{execution-level empirical validation} of the theoretical claims advanced in the Constrained Diffuser framework~\citep{zhang2026constrained}. Whereas prior evaluations measured constraint satisfaction in waypoint space (offline metrics) against explicitly defined analytical boundaries, the present work measures it in action-space execution (online metrics) against the full geometry of the environment. 
...
(Equation~\eqref{eq:phase_lag}). For $\alpha = 0.9$, a typical
smoothing coefficient, the resulting phase margin reduction at
$\omega^* = \pi/4$ is approximately $-18^\circ$, consistent with
the underdamped response observed in
...
$\alpha$ (EMA, disabled)         & EMA coefficient                & N/A    \\
